...
def main():
    ...
    vort = np.ones_like(X)*vort
    R = np.ones_like(X)*R

    points = np.column_stack((X.flatten(),Y.flatten(),vort.flatten(), R.flatten()))
    ...
    u_teo, v_teo, psi_teo = quantities(X, Y, vort, R,)
    ...
    if DO_GIF:
        ...
        for i in range(len(x)):
            if u[i]>0:
                # plt.arrow(x[i],y[i],u[i],v[i], color='forestgreen', scale=scale, width=3e-3, scale_units='inches', alpha=.5, zorder=10)
                plt.arrow(x[i],y[i],u[i]/scale,v[i]/scale, color='forestgreen', width=.02, head_width=0.07, head_length=0.1, length_includes_head=False, alpha=.5, zorder=10)
            else:
                # plt.arrow(x[i],y[i],u[i],v[i], color='firebrick', scale=scale, width=3e-3, scale_units='inches', alpha=.5, zorder=10)
                plt.arrow(x[i]-u[i]/scale+.1,y[i],u[i]/scale,v[i]/scale, color='firebrick', width=.02, head_width=0.07, head_length=0.1, length_includes_head=False, alpha=.5, zorder=10)
        ...
        fname = 'imgs/streamlines_target_fraenkel_fixed.gif'
